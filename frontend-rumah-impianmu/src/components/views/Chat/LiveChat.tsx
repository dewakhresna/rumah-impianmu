...
export default function LiveChat() {
  const { messages, input, setInput, isLoading, messagesEndRef, handleSendMessage } = useChat();
      ...
      <Card className="flex h-full flex-col rounded-2xl border border-slate-100 bg-white shadow-xl shadow-slate-200/50">
        ...
        <CardHeader className="flex items-center gap-4 rounded-t-2xl bg-blue-600 px-6 py-4">
          ...
          <div className="flex flex-col">
            <h4 className="font-semibold leading-tight text-white">Assistant Rumah Impianmu</h4>
            <span className="text-xs text-blue-100">Rekomendasi Rumah Terbaik</span>
          </div>
        </CardHeader>
